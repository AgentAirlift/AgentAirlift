...
/// Lines from assistant turns that look like decisions or commands.
fn extract_decisions(turns: &[CanonicalTurn]) -> Vec<String> {
    ...
    for t in turns.iter().filter(|t| t.role == "assistant") {
        for line in t.content.lines() {
            let l = line.trim();
            if l.starts_with("Decision:") || l.starts_with("decision:") {
                out.push(format!("- {}", l));
            }
        }
    }
...
fn extract_commands(turns: &[CanonicalTurn]) -> Vec<String> {
    ...
    for t in turns {
        for line in t.content.lines() {
            let l = line.trim();
            if l.starts_with("$ ") || l.starts_with("```bash") || l.starts_with("cargo ") || l.starts_with("npm ") || l.starts_with("git ") {
                let cmd = l.trim_start_matches("$ ").trim_start_matches("```bash").trim();
                if !cmd.is_empty() && !out.contains(&format!("- `{}`", cmd)) {
                    out.push(format!("- `{}`", cmd));
                }
            }
        }
