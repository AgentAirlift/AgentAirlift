...
fn migration_pipeline_enforces_ci_gate_invariants() {
    ...
    assert!(!home.join(".codex").exists(), "native install must be opt-in");
    assert!(!home.join(".claude").exists(), "native install must be opt-in");
    ...
    assert_eq!(
        ...
        diagnostics["mapped_records"].as_u64().unwrap()
            + diagnostics["intentionally_skipped_records"].as_u64().unwrap()
    ...
    assert!(!canonical_turns.is_empty(), "canonical session must not be empty");
    ...
    assert!(
        out.join("exports/native/claude-code")
            .read_dir()
